...
Upload an Excel/CSV file (with a 'Style' column: Growth or Value), or enter tickers manually.
""")
...
with st.sidebar:
    ...
    if upload_option == "Upload Excel/CSV File":
        uploaded_file = st.file_uploader(
            "Upload Excel/CSV file with tickers and style",
            type=["xlsx", "xls", "csv"],
            help="File must contain columns: 'Ticker' and 'Style' (Growth/Value)."
        )
    else:
        manual_tickers = st.text_area(
            "Enter tickers (one per line, format: TICKER,STYLE):",
...
def analyze_styles(tickers_df, timeframes):
    if tickers_df.empty:
    ...
    try:
        ...
        if upload_option == "Upload Excel/CSV File" and uploaded_file is not None:
            try:
                ...
                # Require Ticker and Style columns
                if all(col in df.columns for col in ['Ticker', 'Style']):
                    tickers_df = df[['Ticker', 'Style']].dropna()
                    tickers_df['Ticker'] = tickers_df['Ticker'].astype(str).str.upper()
                    tickers_df['Style'] = tickers_df['Style'].str.capitalize()
                    tickers_df = tickers_df[tickers_df['Style'].isin(['Growth', 'Value'])]
                else:
                    st.error("Your file must have columns 'Ticker' and 'Style' (with values Growth or Value).")
            ...
            for line in manual_tickers.strip().split('\n'):
                ...
                if len(parts) == 2 and parts[1].capitalize() in ["Growth", "Value"]:
                    manual_list.append({'Ticker': parts[0].upper(), 'Style': parts[1].capitalize()})
            if manual_list:
                tickers_df = pd.DataFrame(manual_list)
        # 2. Check input validity
        if tickers_df.empty:
            st.warning("Please provide valid tickers with style (Growth/Value)")
        elif not timeframes:
            st.warning("Please select at least one timeframe")
        else:
            with st.spinner("Analyzing performance..."):
                results_df = analyze_styles(tickers_df, timeframes)
...
if not analyze_button:
    st.info("""
    **How to use this app:**
    1. Upload an Excel/CSV file with columns 'Ticker' and 'Style' (Growth/Value)
       OR enter manually (format: TICKER,STYLE per line)
    2. Select the timeframes you want to analyze
    3. Click "Analyze Growth vs Value Performance"

    **Example Tickers (manual entry):**
    AAPL,Growth
    JPM,Value
    GOOGL,Growth
    """)

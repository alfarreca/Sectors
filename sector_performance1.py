        ...
        if not tickers:
            ...
            with st.spinner("Analyzing sector performance..."):
                ...
                if results_df is not None:
                    ...
                    # SECTOR AVERAGES & CHARTS
                    try:
                        numeric_cols = [tf for tf in timeframes if tf in results_df.columns]
                        sector_avg = results_df.groupby('Sector')[numeric_cols].mean(numeric_only=True).reset_index()
                            ...
                            for idx, tf in enumerate(numeric_cols):
                                with cols[idx]:
                                    ...
                                    st.metric(
                                        ...
                                        delta=f"{float(best_sector[tf]):.2f}%"
                                    )
                                    st.metric(
                                        label=f"Worst Sector ({tf})",
                                        value=worst_sector['Sector'],
                                        delta=f"{float(worst_sector[tf]):.2f}%"
                                    )
                    except Exception as e:
...
if not analyze_button:
    ...
    **How to use this app:**
    ...
    2. Select the timeframes you want to analyze (including '3 Months')
